...
function createFileSessionStorage({
  ...
  return createSessionStorage({
    ...
    async createData(data, expires) {
      ...
      while (true) {
        let randomBytes = crypto.getRandomValues(new Uint8Array(8)); // This storage manages an id space of 2^64 ids, which is far greater
        ...
        try {
          ...
          let exists = await Deno.stat(file).then(s => s.isFile).catch(() => false);
          if (exists) continue;
          await Deno.mkdir(path.dirname(file), {
            recursive: true
          }).catch(() => {});
...
export { createFileSessionStorage };
